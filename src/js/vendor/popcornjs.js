...
(function (p, e) {
	...
	function b(a, c) {
		return function () {
			...
			try {
				...
				f.plugin.errors.push({plugin: c, thrown: n, source: a.toString()});
	...
	if (e.addEventListener) {
		var h = Array.prototype,
			i = Object.prototype, g = h.forEach, k = h.slice, r = i.hasOwnProperty, m = i.toString, t = p.Popcorn, q = [], o = false, u = {
				events: {
					hash: {},
					apis: {}
				}
		...
		f.p = f.prototype =
		{
			init: function (a, c) {
				var n, j = this;
				if (typeof a === "function")if (e.readyState === "complete")a(e, f); else {
					q.push(a);
					if (!o) {
						o = true;
						var w = function () {
					...
					var z = function () {
						...
						f.addTrackEvent(j, {start: F, end: F});
						if (!j.isDestroyed) {
							j.data.durationChange = function () {
								var B = j.media.duration, Q = B + 1, K = j.data.trackEvents.byStart, M = j.data.trackEvents.byEnd;
								K.pop();
								M.pop();
								for (var D = M.length - 1; D > 0; D--)M[D].end > B && j.removeTrackEvent(M[D]._id);
								for (M = 0; M < K.length; M++)K[M].end > B && j.removeTrackEvent(K[M]._id);
								j.data.trackEvents.byEnd.push({start: Q, end: Q});
								j.data.trackEvents.byStart.push({start: Q, end: Q})
							};
		...
		f.byId = function (a) {
			for (var c = f.instances, n = c.length, j = 0; j < n; j++)if (c[j].id === a)return c[j];
			return null
		};
		...
		f.extend(f, {
			noConflict: function (a) {
				...
				var c = {}, n = e.documentElement, j = e.body, w, x, z;
				...
				return f.extend({}, c, {top: w, left: x})
			}, disable: function (a, c) {
				if (!a.data.disabled[c]) {
					a.data.disabled[c] = true;
					if (c in f.registryByName && a.data.running[c])for (var n = a.data.running[c].length - 1, j; n >= 0; n--) {
						j = a.data.running[c][n];
						j._natives.end.call(a,
							null, j);
						a.emit("trackend", f.extend({}, j, {plugin: j.type, type: "trackend"}))
					}
					return a
				}
			}, enable: function (a, c) {
				if (a.data.disabled[c]) {
					...
					if (c in f.registryByName && a.data.running[c])for (var n = a.data.running[c].length - 1, j; n >= 0; n--) {
						...
						a.emit("trackstart", f.extend({}, j, {plugin: j.type, type: "trackstart", track: j}))
					}
					return a
				}
			}, destroy: function (a) {
				var c = a.data.events, n = a.data.trackEvents, j, w, x, z;
				for (w in c) {
					j = c[w];
				...
				function (c) {
					a[c] = function (n) {
						...
						if (n != null) {
							j = this.media[c];
							this.media[c] = n;
							j !== n && this.emit("attrchange", {attribute: c, previousValue: j, currentValue: n});
							return this
						}
						return this.media[c]
					}
		...
		f.extend(f.p, {
			...
			exec: function (a, c, n) {
				var j = arguments.length, w = "trackadded", x, z;
				try {
				...
				if (typeof a === "number" && j === 2) {
					...
					c = a;
					a = f.guid("cue")
				} else if (j === 1)c = -1; else if (x = this.getTrackEvent(a)) {
				...
				j = {
					...
					_running: false,
					_natives: {start: n || f.nop, end: f.nop, type: "cue"}
				};
				if (x)j = f.extend(x, j);
				if (w === "cuechange") {
					...
					this.emit(w, f.extend({}, j, {
						id: a,
						type: w,
						previousValue: {time: x.start, fn: x._natives.start},
						currentValue: {time: c, fn: n || f.nop},
						track: x
					}))
		...
		f.events = {
			isNative: function (a) {
				...
				return x
			}, all: f.Events.Natives.split(/\s+/g), fn: {
				trigger: function (a, c) {
					...
					var n = this, j = true, w = f.events.hooks[a], x, z;
					...
					!j && f.events.all.indexOf(a) > -1 && this.media.addEventListener(a, function (F) {
						if (n.data.events[a])for (x = n.data.events[a].slice(); x.length;)x.shift().call(n, F)
					}, false);
					return this
				...
				canplayall: {
					bind: "canplaythrough", add: function (a, c) {
						var n = false;
		...
		f.forEach([["trigger", "emit"], ["listen", "on"], ["unlisten", "off"]], function (a) {
		...
		l.start = function (a, c) {
			if (c.end > a.media.currentTime && c.start <= a.media.currentTime && !c._running) {
				...
				if (!a.data.disabled[c._natives.type]) {
					...
					a.emit("trackstart", f.extend({}, c, {plugin: c._natives.type, type: "trackstart", track: c}))
				}
			}
		};
		l.end = function (a, c) {
			...
			if ((c.end <= a.media.currentTime || c.start >
				...
				if (!a.data.disabled[c._natives.type]) {
					...
					a.emit("trackend", f.extend({}, c, {plugin: c._natives.type, type: "trackend", track: c}))
				}
			}
		};
		...
		d.prototype.add = function (a) {
			...
			for (j = c.length - 1; j >= 0; j--)if (a.start >= c[j].start) {
				c.splice(j + 1, 0, a);
				break
			}
			for (c = n.length - 1; c >= 0; c--)if (a.end > n[c].end) {
				n.splice(c + 1, 0, a);
				break
			}
			...
			if (!(c instanceof l)) {
				...
				if (c._natives) {
					...
					if (c._natives._setup) {
						...
						a.emit("tracksetup", f.extend({}, c, {plugin: c._natives.type, type: "tracksetup", track: c}))
					}
				}
				...
				a.emit("trackadded", f.extend({}, c, c._natives ? {plugin: c._natives.type} : {}, {
					type: "trackadded",
					track: c
				}))
			}
		...
		f.timeUpdate = function (a, c) {
			var n = a.media.currentTime, j = a.data.trackEvents.previousUpdateTime, w = a.data.trackEvents, x = w.endIndex, z = w.startIndex, F = w.byStart.length, v = w.byEnd.length, L = f.registryByName, y, s, B;
			if (j <= n) {
					...
					if (!j || L[s] || a[s]) {
						if (y._running === true) {
							...
							if (!a.data.disabled[s]) {
								j.end.call(a, c, y);
								a.emit("trackend", f.extend({}, y, {plugin: s, type: "trackend", track: y}))
							}
						}
						x++
					} else {
					...
					if (!j || L[s] || a[s]) {
						if (y.end > n && y._running === false) {
							...
							if (!a.data.disabled[s]) {
								j.start.call(a, c,
									y);
								a.emit("trackstart", f.extend({}, y, {plugin: s, type: "trackstart", track: y}))
							}
						}
						z++
					} else {
		...
		f.plugin = function (a, c, n) {
			if (f.protect.natives.indexOf(a.toLowerCase()) >=
				0)f.error("'" + a + "' is a protected function name"); else {
				var j = typeof c === "function", w = ["start", "end", "type", "manifest"], x = ["_setup", "_teardown", "start", "end", "frame"], z = {}, F = function (y, s) {
					y = y || f.nop;
					s = s || f.nop;
					return function () {
				...
				var v = function (y, s) {
					...
					B._teardown = F(function () {
							var M = k.call(arguments), D = this.data.running[B.type];
							M.unshift(null);
				...
				f.p[a] = z[a] = function (y, s) {
					var B, Q;
					if (y && !s)s = y; else if (B = this.getTrackEvent(y)) {
						Q = s;
						var K = {}, M;
						for (M in B)if (r.call(Q, M) && r.call(B, M))K[M] = B[M];
						if (B._natives._update) {
							...
							if (j)v.call(this, c.call(this, B), B); else {
								...
								if (B._natives && B._natives._setup) {
									...
									this.emit("tracksetup", f.extend({}, B,
										{plugin: B._natives.type, type: "tracksetup", track: B}))
								}
								this.data.trackEvents.add(B);
								l.start(this, B);
								this.timeUpdate(this, null, true);
					...
					B = f.extend({}, this.options.defaults && this.options.defaults[a] ||
						{}, s);
					v.call(this, j ? c.call(this, B) : c, B);
					return this
				};
				n && f.extend(c, {manifest: n});
				var L = {fn: z[a], definition: c, base: c, parents: [], name: a};
				f.registry.push(f.extend(z, L, {type: a}));
		...
		f.removePlugin = function (a, c) {
			if (!c) {
				...
				var n = f.registry.length, j;
				for (j = 0; j < n; j++)if (f.registry[j].name === c) {
					f.registry.splice(j,
						1);
			...
			var w = a.data.trackEvents.animating, x, z;
			x = 0;
			...
			x = 0;
			for (z = w.length; x <
			z; x++)if (w[x] && w[x]._natives && w[x]._natives.type === c) {
				w.splice(x, 1);
				x--;
				z--
			}
		...
		var A = /\?/, O = {
			ajax: null, url: "", data: "", dataType: "", success: f.nop, type: "GET",
			async: true, contentType: "application/x-www-form-urlencoded; charset=UTF-8"
		};
		f.xhr = function (a) {
			a.dataType = a.dataType && a.dataType.toLowerCase() || null;
			if (a.dataType && (a.dataType === "jsonp" || a.dataType === "script"))f.xhr.getJSONP(a.url, a.success, a.dataType === "script"); else {
				a = f.extend({}, O, a);
		...
		f.xhr.httpData = function (a) {
			...
			a.ajax.onreadystatechange = function () {
				if (a.ajax.readyState === 4) {
					...
					c = {xml: a.ajax.responseXML, text: a.ajax.responseText, json: n};
		...
		f.xhr.getJSONP = function (a, c, n) {
			var j = e.head || e.getElementsByTagName("head")[0] || e.documentElement, w = e.createElement("script"), x = false, z = [];
			z = /(=)\?(?=&|$)|\?\?/;
			var F, v;
			if (!n) {
				v = a.match(/(callback=[^&]*)/);
		...
		f.forEach({listen: "on", unlisten: "off", trigger: "emit", exec: "cue"}, function (a, c) {
			var n = f.p[c];
			f.p[c] = function () {
...
(function (p, e) {
	var l = p.document, d = p.location, b = /:\/\//, h = d.href.replace(d.href.split("/").slice(-1)[0], ""), i = function (k, r, m) {
		k = k || 0;
		r = (r || k || 0) + 1;
		m = m || 1;
		r = Math.ceil((r - k) / m) || 0;
	...
	e.sequence.init = function (k, r) {
		...
		e.forEach(r, function (q, o) {
			...
			m.queue.push(u);
			var E = q["in"], C = q.out;
			m.inOuts.ofVideos.push({"in": E !== undefined && E || 1, out: C !== undefined && C || 0});
	...
	e.sequence.cycle = function (k) {
		...
		var r = this.queue, m = this.inOuts.ofVideos, t = r[k], q = 0, o;
		if (r[k + 1])q = k + 1;
		if (r[k + 1]) {
			r = r[q];
			m = m[q];
			e.extend(r, {width: this.dims.width, height: this.dims.height});
			o = this.playlist[q];
			t.pause();
			this.active = q;
			...
			o.currentTime(m["in"]);
			o[q ? "play" : "pause"]();
			this.trigger("cycle", {position: {previous: k, current: q}});
			if (q) {
	...
	e.extend(e.sequence.prototype, {
		eq: function (k) {
			...
			e.addTrackEvent(this.playlist[m], {
				...
				_running: false,
				_natives: {start: r || e.nop, end: e.nop, type: "exec"}
			});
			return this
		}, listen: function (k, r) {
			var m = this, t = this.playlist, q = t.length, o = 0;
			if (!r)r = e.nop;
			if (e.Events.Natives.indexOf(k) > -1)e.forEach(t, function (u) {
				u.listen(k, function (E) {
					E.active = m;
					if (g.indexOf(k) > -1)r.call(u, E); else++o === q && r.call(u, E)
				})
			}); else {
	...
	e.forEach(e.manifest, function (k, r) {
		e.sequence.prototype[r] = function (m) {
			...
			for (o = 0; o < this.inOuts.ofClips.length; o++) {
				...
				E = u.indexOf(m.start);
				C = u.indexOf(m.end);
				if (E > -1)t[o] = e.extend({}, q, {start: u[E], clipIdx: E});
				if (C > -1)t[o] =
					e.extend({}, q, {end: u[C], clipIdx: C})
			}
...
(function (p, e) {
	function l(h) {
		...
		var i = h.split("-");
		return {iso6391: h, language: i[0] || "", region: i[1] || ""}
	}
	
	var d = p.navigator, b = l(d.userLanguage || d.language);
	e.locale = {
		get: function () {
			return b
		}, set: function (h) {
			...
			var i = e.instances, g = i.length, k = 0, r;
...
(function (p) {
	...
	p.player = function (l, d) {
		if (!p[l]) {
			...
			var b = function (h, i, g) {
				g = g || {};
				var k = new Date / 1E3, r = k, m = 0, t = 0, q = 1, o = false, u = {}, E = typeof h === "string" ? p.dom.find(h) : h, C = {};
				Object.prototype.__defineGetter__ || (C = E || document.createElement("div"));
				for (var f in E)if (!(f in C))if (typeof E[f] === "object")C[f] = E[f]; else if (typeof E[f] === "function")C[f] = function (A) {
					return "length"in E[A] && !E[A].call ?
						E[A] : function () {
						return E[A].apply(E, arguments)
					}
				}(f); else p.player.defineProperty(C, f, {
					get: function (A) {
						return function () {
							return E[A]
						}
					}(f), set: p.nop, configurable: true
				});
				var G = function () {
					k = new Date / 1E3;
					if (!C.paused) {
				...
				C.dispatchEvent = function (A) {
					...
					if (u[a])for (A = u[a].length - 1; A >= 0; A--)u[a][A].call(this, O, this)
				};
				C.src = i || "";
				C.duration = 0;
				C.paused = true;
				...
				if (d._canPlayType(E.nodeName,
						i) !== false)if (d._setup)d._setup.call(C, g); else {
					C.readyState = 4;
	...
	p.smart = function (l, d, b) {
		var h = typeof l === "string" ? p.dom.find(l) : l, i, g, k, r, m, t = "HTMLYouTubeVideoElement HTMLVimeoVideoElement HTMLSoundCloudAudioElement HTMLNullVideoElement".split(" ");
		if (h) {
			d = typeof d === "string" ? [d] : d;
			l = 0;
			for (m = d.length; l < m; l++) {
				i = d[l];
				for (g = 0; g < t.length; g++)if ((r =
				...
				for (var q in p.player.registry)if (p.player.registry.hasOwnProperty(q))if (p.player.registry[q].canPlayType(h.nodeName, i))return p[q](h, i, b)
			}
			var o;
...
(function (p) {
	...
	p.parser = function (l, d, b) {
		if (p.protect.natives.indexOf(l.toLowerCase()) >= 0)p.error("'" + l + "' is a protected function name"); else {
			...
			if (!(typeof b !== "function" || typeof d !== "string")) {
				...
				h[l] = function (i, g, k) {
					...
					var r = this;
					p.xhr({
						url: i, dataType: d, success: function (m) {
							var t, q, o = 0;
							m = b(m, k).data || [];
							if (t = m.length) {
								for (; o < t; o++) {
...
(function (p, e) {
	...
	var d = {length: 0, start: p.nop, end: p.nop};
	...
	p._MediaElementProto = function () {
		...
		b._util = {
			type: "HTML5", TIMEUPDATE_MS: 250, MIN_WIDTH: 300, MIN_HEIGHT: 150, isAttributeSet: function (i) {
		...
		b.dispatchEvent = function (i) {
			...
			g.initCustomEvent(this._eventNamespace + i, false, false,
				{type: i, target: this.parentNode, data: null});
			e.dispatchEvent(g)
		};
		b.load = p.nop;
	...
	function i(t) {
		...
		function G(P) {
			if (z._canPlaySrc(P)) {
				...
				if (b()) {
					if (L)L && K && K.destroy();
					jwplayer(F.id).setup({file: P, width: "100%", height: "100%", controls: v.controls});
					K = jwplayer(F.id);
					K.onReady(C);
					K.onError(f);
					jwplayer.utils.log = function (H, V) {
						if (typeof console !== "undefined" && typeof console.log !== "undefined")V ? console.log(H, V) : console.log(H);
						H === "No suitable players found and fallback enabled" && f({message: H, code: 4})
					};
		...
		var z = new p._MediaElementProto, F = typeof t === "string" ? l.querySelector(t) : t, v = {
			src: g,
			...
			paused: true,
			error: null
		}, L = false, y = false, s = false, B = false, Q = false, K, M = true, D = [], T = true, I = false;
...
(function (p, e) {
	...
	function d(i) {
		...
		var o = new p._MediaElementProto, u = typeof i === "string" ? e.querySelector(i) : i, E = e.createElement("div"), C = false, f, G = {
			src: b,
		...
		Object.defineProperties(o, {
			src: {
				get: function () {
					...
					if (a && a !== G.src)if (o._canPlaySrc(a)) {
						G.src = a;
						if (C)if (C && f) {
							f.pause();
							f = null;
						...
						u.appendChild(E);
						a = h.exec(a);
						f = new l({currentTime: +a[1], duration: +a[2], endedCallback: q});
...
(function (p, e, l) {
	function d(k) {
		...
		"play pause paused seekTo unload getCurrentTime getDuration getVideoEmbedCode getVideoHeight getVideoWidth getVideoUrl getColor setColor setLoop getVolume setVolume addEventListener".split(" ").forEach(function (t) {
			r[t] = function (q) {
				q = JSON.stringify({method: t, value: q});
	...
	function b(k) {
		...
		var A = new p._MediaElementProto, O = typeof k === "string" ? p.dom.find(k) : k, a = l.createElement("iframe"), c = {
				src: i,
		...
		Object.defineProperties(A, {
			src: {
				get: function () {
					...
					if (y && y !== c.src)if (A._canPlaySrc(y)) {
						c.src = y;
						if (n)if (n && w) {
							clearInterval(v);
							w.pause();
						...
						var s = y.queryKey, B, Q = ["api=1", "player_id=" + j, "title=0", "byline=0", "portrait=0"];
	...
	var h = 16, i = "", g = "https://player.vimeo.com";
...
(function (p, e) {
	...
	function d(b, h) {
		var i = typeof b === "string" ? e.querySelector(b) : b, g = e.createElement(h);
		i.appendChild(g);
		g._canPlaySrc = l;
		return g
	}
...
(function (p, e, l) {
	function d() {
		var u;
		if (YT.loaded)for (t = true; o.length;) {
			u = o.shift();
			u()
	...
	function i(u) {
		...
		function a() {
			j("pause", a);
			if (R.getCurrentTime() > 0)setTimeout(a, 0); else if (H.autoplay || !H.paused) {
				n("play", O);
				R.playVideo()
			} else O()
		}
		...
		function c() {
			j("play", c);
			if (R.getCurrentTime() === 0)setTimeout(c, 0); else {
				n("pause", a);
				R.seekTo(0);
				R.pauseVideo()
			}
			...
			if (I._canPlaySrc(J)) {
				...
				if (b()) {
					...
					p.getJSONP("https://gdata.youtube.com/feeds/api/videos/" + J + "?v=2&alt=jsonc&callback=?", function (X) {
						if (X.error)console.warn("failed to retreive duration data, reason: " + X.error.message); else if (X.data) {
							H.duration = X.data.duration;
							I.dispatchEvent("durationchange");
							R = new YT.Player(S, {
								width: "100%", height: "100%", wmode: N.wmode, videoId: J, playerVars: N,
								events: {onReady: G, onError: A, onStateChange: z}
							});
							H.networkState = I.NETWORK_LOADING;
		...
		function v() {
			...
			if (H.seeking)m(J - H.currentTime) < 1 && Q(); else {
		...
		var I = new p._MediaElementProto, P = typeof u === "string" ? l.querySelector(u) : u, S = l.createElement("div"), H = {
			src: k,
			...
			paused: true,
			error: null
		}, V = false, U = false, Z = false, R, Y = true, W = [], ca = -1, ba, $ = 0, aa, da;
	...
	var g = 10, k = "", r = /^.*(?:\/|v=)(.{11})/, m = Math.abs, t = false, q = false, o = [];
		...
		if (!r) {
			p.getScript("https://w.soundcloud.com/player/api.js", function () {
				p.getScript("https://connect.soundcloud.com/sdk.js", function () {
					k = true;
					SC.initialize({client_id: "PRaNFlda6Bhf5utPjUsptg"});
					for (var t = m.length; t--;) {
						m[t]();
						delete m[t]
					}
	...
	function h(t) {
		...
		function o() {
			s.bind(SC.Widget.Events.LOAD_PROGRESS, function (D) {
				O({type: "loadProgress", data: D.currentPosition / 1E3})
			});
			s.bind(SC.Widget.Events.PLAY_PROGRESS,
				function (D) {
					O({type: "playProgress", data: D.currentPosition / 1E3})
				});
			...
			s.bind(SC.Widget.Events.SEEK, function () {
				s.getPosition(function (D) {
					D = D / 1E3;
					if (v.seeking)if (Math.floor(D) !== Math.floor(v.currentTime))s.seekTo(v.currentTime * 1E3); else {
						v.ended = false;
						v.seeking = false;
		...
		function c(D) {
			if (x._canPlaySrc(D)) {
				...
				if (d()) {
					L = false;
					SC.get("/resolve", {url: D}, function (T) {
						var I;
						if (T.errors) {
							I = {name: "MediaError"};
							if (T.errors[0])if (T.errors[0].error_message === "404 - Not Found") {
		...
		var x = new p._MediaElementProto, z = typeof t === "string" ? p.dom.find(t) : t, F = l.createElement("iframe"), v = {
			src: g,
...
(function (p) {
	var e = function (l, d) {
		...
		p.forEach(d.classes, function (g, k) {
			i = [];
			if (g === "parent")i[0] = document.querySelectorAll("#" + d.target)[0].parentNode; else i = document.querySelectorAll("#" + d.target + " " + g);
			b = 0;
...
(function (p) {
	function e(d, b) {
		if (d.map)d.map.div.style.display = b; else setTimeout(function () {
			e(d, b)
		}, 10)
	}
	...
	p.plugin("openmap", function (d) {
			...
			if (window.OpenLayers && window.OpenLayers.Layer.Stamen) {
				...
				switch (d.type) {
					case "SATELLITE":
						...
						var o = new OpenLayers.Layer.WorldWind("LANDSAT", "//worldwind25.arc.nasa.gov/tile/tile.aspx", 2.25, 4, {T: "105"});
						d.map.addLayer(o);
					...
					case "TERRAIN":
						g = new OpenLayers.Projection("EPSG:4326");
						i = new OpenLayers.Projection("EPSG:4326");
						d.map = new OpenLayers.Map({div: b, projection: i});
						o = new OpenLayers.Layer.WMS("USGS Terraserver", "//terraserver-usa.org/ogcmap.ashx?", {layers: "DRG"});
						d.map.addLayer(o);
						break;
					case "STAMEN-TONER":
					...
					default:
						...
						g = new OpenLayers.Projection("EPSG:4326");
						h = h.transform(g, i);
						d.map = new OpenLayers.Map({div: b, projection: i, displayProjection: g});
			...
			_setup: function (o) {
				...
				var u = function () {
					if (o.map) {
						...
						if (o.markers) {
							var E = OpenLayers.Util.extend({}, OpenLayers.Feature.Vector.style["default"]), C = function (j) {
								...
								p.getJSONP("//tinygeocoder.com/create-api.php?q=" + j.location + "&callback=jsonp", function (w) {
									...
									if (j.text)w.attributes = {text: j.text};
									k.addFeatures([w])
								})
							};
							k = new OpenLayers.Layer.Vector("Point Layer", {style: E});
							o.map.addLayer(k);
							for (var A = 0, O = o.markers.length; A < O; A++) {
								...
								if (a.text)if (!r) {
									...
									o.map.addControl(r);
									r.activate();
									k.events.on({featureselected: C, featureunselected: f})
								}
								if (a.location)G(a);
								else {
									var c = (new OpenLayers.Geometry.Point(a.lng, a.lat)).transform(g, i), n = OpenLayers.Util.extend({}, E);
									if (!a.size || isNaN(a.size))a.size = 14;
									n.pointRadius = a.size;
									n.graphicOpacity = 1;
									n.externalGraphic = a.icon;
									c = new OpenLayers.Feature.Vector(c, null, n);
									if (a.text)c.attributes = {text: a.text};
									k.addFeatures([c])
								}
							}
						}
		...
		about: {
			...
			version: "0.3", author: "@mapmeld", website: "mapadelsur.blogspot.com"
		}, options: {
			start: {elem: "input", type: "number", label: "Start"},
			end: {elem: "input", type: "number", label: "End"},
			target: "map-container",
			type: {elem: "select", options: ["ROADMAP", "SATELLITE", "TERRAIN"], label: "Map Type", optional: true},
			zoom: {elem: "input", type: "number", label: "Zoom", "default": 2},
			lat: {elem: "input", type: "text", label: "Lat", optional: true},
			lng: {elem: "input", type: "text", label: "Lng", optional: true},
			location: {
				elem: "input", type: "text", label: "Location",
	...
	p.plugin("wikipedia", {
		manifest: {
			about: {
				...
				lang: {elem: "input", type: "text", label: "Language", "default": "english", optional: true},
				src: {
					elem: "input",
					type: "url",
					label: "Wikipedia URL",
				...
				title: {elem: "input", type: "text", label: "Title", "default": "Cats", optional: true},
				numberofwords: {
					elem: "input",
					type: "number",
			...
			var d = function () {
				if (l._fired) {
					if (l._link &&
						l._desc)if (document.getElementById(l.target)) {
...
(function (p) {
	var e = 0, l = function (d, b) {
		var h = d.container = document.createElement("div"), i = h.style, g = d.media, k = function () {
			var r = d.position();
			i.fontSize = "18px";
	...
	p.plugin("subtitle", {
		manifest: {
			about: {
				...
				version: "0.1", author: "Scott Downe", website: "http://scottdowne.wordpress.com/"
			},
			options: {
	...
	p.plugin("documentcloud", {
		manifest: {
			about: {
				...
				width: {elem: "input", type: "text", label: "Width", optional: true},
				...
				preload: {elem: "input", type: "checkbox", label: "Preload", "default": true},
				page: {elem: "input", type: "number", label: "Page Number", optional: true},
			...
			function b() {
				...
				function t() {
					l[d._key] = {num: 1, id: d._containerId};
					i.loaded = true
				}
				
				i.loaded = false;
				var q = d.url.replace(/\.html$/, ".js"),
					o = d.target, u = e.getElementById(o), E = e.createElement("div"), C = p.position(u), f = d.width || C.width;
				C = d.height || C.height;
				var G = d.sidebar || true, A = d.text || true, O = d.pdf || true, a = d.showAnnotations || true, c = d.zoom || 700, n = d.search || true;
				if (!function (j) {
						var w = false;
			...
			if (i.loading)h(); else {
				...
				var k = e.createElement("link"), r = e.getElementsByTagName("head")[0];
				k.rel = "stylesheet";
				k.type = "text/css";
				k.media = "screen";
			...
			var h = e.getElementById(b._containerId), i = DV.viewers[b._key];
...
(function (p) {
	var e = /(?:http:\/\/www\.|http:\/\/|www\.|\.|^)(youtu|vimeo|soundcloud|baseplayer)/, l = {}, d = {
		vimeo: false,
		youtube: false,
		soundcloud: false,
		module: false
	...
	p.plugin("mediaspawner", {
		manifest: {
			about: {
				...
				autoplay: {elem: "input", type: "checkbox", label: "Autoplay Video", optional: true},
				width: {
					elem: "input",
					type: "number",
					label: "Media Width",
...
(function (p) {
	...
	p.plugin("timeline", function (l) {
		var d = document.getElementById(l.target), b = document.createElement("div"), h, i = true;
		if (d && !d.firstChild) {
		...
		about: {
			...
			text: {elem: "input", type: "text", label: "Text"},
			innerHTML: {elem: "input", type: "text", label: "HTML Code", optional: true},
...
(function (p) {
	...
	p.plugin("flickr", function (l) {
		var d, b = document.getElementById(l.target), h, i, g, k, r = l.numberofimages || 4, m = l.height || "50px", t = l.width || "50px", q = l.padding || "5px", o = l.border || "0px";
		...
		about: {
			...
			userid: {elem: "input", type: "text", label: "User ID", optional: true},
			tags: {elem: "input", type: "text", label: "Tags"},
			username: {elem: "input", type: "text", label: "Username", optional: true},
			apikey: {
				elem: "input", type: "text",
				label: "API Key", optional: true
			},
			target: "flickr-container",
			height: {elem: "input", type: "text", label: "Height", "default": "50px", optional: true},
			width: {elem: "input", type: "text", label: "Width", "default": "50px", optional: true},
			padding: {elem: "input", type: "text", label: "Padding", optional: true},
			border: {elem: "input", type: "text", label: "Border", "default": "5px", optional: true},
			numberofimages: {elem: "input", type: "number", "default": 4, label: "Number of Images"}
		}
	})
})(Popcorn);
(function (p) {
	p.plugin("webpage", {
		manifest: {
			...
			options: {
				id: {elem: "input", type: "text", label: "Id", optional: true},
				start: {elem: "input", type: "number", label: "Start"},
				end: {elem: "input", type: "number", label: "End"},
				src: {elem: "input", type: "url", label: "Webpage URL", "default": "http://mozillapopcorn.org"},
...
(function (p) {
	var e = {}, l = 0, d = document.createElement("span"), b = ["webkit", "Moz", "ms", "O", ""], h = ["Transform", "TransitionDuration", "TransitionTimingFunction"], i = {}, g;
	document.getElementsByTagName("head")[0].appendChild(d);
	for (var k = 0, r = h.length; k < r; k++)for (var m = 0, t = b.length; m < t; m++) {
		g = b[m] + h[k];
		if (g in d.style) {
			i[h[k].toLowerCase()] = g;
			break
	...
	p.plugin("wordriver", {
		manifest: {
			about: {name: "Popcorn WordRiver Plugin"}, options: {
				start: {
				...
				text: {elem: "input", type: "text", label: "Text", "default": "Popcorn.js"},
				color: {elem: "input", type: "text", label: "Color", "default": "Green", optional: true}
			}
		}, _setup: function (q) {
...
(function (p) {
	...
	p.plugin("googlemap", function (g) {
		...
		var q = function () {
			if (b) {
				if (k)if (g.location)h.geocode({address: g.location}, function (o, u) {
					...
					if (r) {
						...
						if (u.type === "STREETVIEW") {
							...
							if (u.location && typeof u.tween === "string") {
								var A = C, O = [], a = new google.maps.DirectionsService, c = new google.maps.DirectionsRenderer(A);
								a.route({
									origin: u.location,
									destination: u.tween,
		...
		about: {
			...
			start: {elem: "input", type: "start", label: "Start"},
			end: {elem: "input", type: "start", label: "End"},
			target: "map-container",
			type: {
				elem: "select",
			...
			zoom: {elem: "input", type: "text", label: "Zoom", "default": 0, optional: true},
			lat: {
				elem: "input",
...
(function (p) {
	p.plugin("mustache", function (e) {
		...
		var i = !!e.dynamic, g = typeof e.template, k = typeof e.data, r = document.getElementById(e.target);
		e.container = r || document.createElement("div");
		if (g === "function")if (i)b = e.template; else h = e.template(e); else h = g === "string" ? e.template : "";
		if (k === "function")if (i)l = e.data; else d = e.data(e); else d = k === "string" ? JSON.parse(e.data) : k === "object" ? e.data : "";
		return {
			start: function (m, t) {
				var q = function () {
					if (window.Mustache) {
		...
		about: {
			...
			dynamic: {elem: "input", type: "checkbox", label: "Dynamic", "default": true}
		}
	})
})(Popcorn);
...
(function (p) {
	...
	p.plugin("googlefeed", function (d) {
		var b = function () {
			var k = false, r = 0, m = document.getElementsByTagName("link"), t = m.length, q = document.head || document.getElementsByTagName("head")[0], o = document.createElement("link");
			if (window.GFdynamicFeedControl)l = true; else p.getScript("//www.google.com/uds/solutions/dynamicfeed/gfdynamicfeedcontrol.js", function () {
				l = true
			});
			for (; r < t; r++)if (m[r].href === "//www.google.com/uds/solutions/dynamicfeed/gfdynamicfeedcontrol.css")k = true;
			if (!k) {
				o.type =
					"text/css";
				o.rel = "stylesheet";
		...
		var h = document.createElement("div"), i = document.getElementById(d.target), g = function () {
		...
		about: {
			...
			target: "feed-container",
			url: {elem: "input", type: "url", label: "Feed URL", "default": "http://planet.mozilla.org/rss20.xml"},
			title: {
				elem: "input", type: "text",
...
(function (p) {
	...
	function l(b, h) {
		var i = b.container = document.createElement("div"), g = i.style, k = b.media, r = function () {
			var m = b.position();
			g.fontSize = "18px";
	...
	p.plugin("text", {
		manifest: {
			about: {name: "Popcorn Text Plugin", version: "0.1", author: "@humphd"},
			options: {
				start: {elem: "input", type: "number", label: "Start"},
				end: {elem: "input", type: "number", label: "End"},
				text: {elem: "input", type: "text", label: "Text", "default": "Popcorn.js"},
				escape: {elem: "input", type: "checkbox", label: "Escape"},
				multiline: {elem: "input", type: "checkbox", label: "Multiline"}
			}
		}, _setup: function (b) {
			var h, i, g = b._container = document.createElement("div");
			g.style.display = "none";
			if (b.target)if (h = p.dom.find(b.target)) {
				if (["VIDEO", "AUDIO"].indexOf(h.nodeName) > -1)h = l(this, b.target + "-overlay")
			} else h = l(this, b.target); else h = this.container ? this.container : l(this);
			b._target = h;
...
(function (p) {
	p.plugin("code", function (e) {
		...
		about: {
			...
			onFrame: {
				elem: "input", type: "function", label: "onFrame",
				optional: true
			},
	...
	p.plugin("image", {
		manifest: {
			about: {
				...
				text: {elem: "input", type: "text", label: "Caption", "default": "Popcorn.js", optional: true}
			}
		}, _setup: function (b) {
			var h = document.createElement("img"), i = document.getElementById(b.target);
			...
			if (i)if (["VIDEO", "AUDIO"].indexOf(i.nodeName) > -1) {
			...
			h.src = b.src;
			b.toString = function () {
				var g = b.src || b._natives.manifest.options.src["default"], k = g.replace(/.*\//g, "");
				return k.length ? k : g
			}
		}, start: function (b, h) {
			...
			if (b.trackedContainer)b.trackedContainer.destroy(); else b.anchor.parentNode && b.anchor.parentNode.removeChild(b.anchor)
		}
	})
})(Popcorn);
(function (p) {
	p.parser("parseXML", "XML", function (e) {
		var l = {title: "", remote: "", data: []}, d = {}, b = function (m) {
			m = m.split(":");
			if (m.length === 1)return parseFloat(m[0], 10); else if (m.length === 2)return parseFloat(m[0], 10) + parseFloat(m[1] / 12, 10); else if (m.length === 3)return parseInt(m[0] * 60, 10) + parseFloat(m[1], 10) + parseFloat(m[2] / 12, 10); else if (m.length === 4)return parseInt(m[0] * 3600, 10) + parseInt(m[1] * 60, 10) + parseFloat(m[2], 10) + parseFloat(m[3] / 12, 10)
		}, h = function (m) {
			for (var t = {}, q = 0, o = m.length; q < o; q++) {
				var u = m.item(q).nodeName,
					E = m.item(q).nodeValue, C = d[E];
				if (u === "in")t.start = b(E); else if (u === "out")t.end = b(E); else if (u === "resourceid")for (var f in C) {
					if (C.hasOwnProperty(f))if (!t[f] && f !== "id")t[f] = C[f]
				} else t[u] = E
			}
			return t
		}, i = function (m, t) {
			...
			var o = {};
			p.extend(o, t, h(m.attributes), {text: m.textContent || m.text});
			t = m.childNodes;
			if (t.length < 1 || t.length === 1 && t[0].nodeType === 3)if (q)d[o.id] = o; else l.data.push(i(m.nodeName, o)); else for (m = 0; m < t.length; m++)t[m].nodeType === 1 && g(t[m], o, q)
		};
		e = e.documentElement.childNodes;
		for (var k = 0, r = e.length; k < r; k++)if (e[k].nodeType === 1)e[k].nodeName === "manifest" ? g(e[k], {}, true) : g(e[k], {}, false);
		return l
	})
})(Popcorn);
(function (p) {
	p.parser("parseSBV", function (e) {
		var l = {title: "", remote: "", data: []}, d = [], b = 0, h = 0, i = function (q) {
			q = q.split(":");
			var o = q.length - 1, u;
			try {
...
(function (p) {
	p.parser("parseTTXT", function (e) {
		var l = {title: "", remote: "", data: []}, d = function (k) {
			k = k.split(":");
			var r = 0;
			try {
...
(function (p) {
	function e(g, k, r) {
		...
		r = [];
		for (var t; m;) {
			if (m.nodeType === 1)if (m.nodeName === "p")r.push(d(m, k, g)); else if (m.nodeName === "div") {
	...
	function d(g, k, r) {
		...
		if (m.end < 0) {
			...
			if (m.end >= 0)m.end += m.start; else m.end = Number.MAX_VALUE
		}
		return {subtitle: m}
	}
	...
	function b(g, k) {
		...
		try {
			...
			r = g.substring(r);
			return t * ({h: 3600, m: 60, s: 1, ms: 0.0010}[r] || -1) + (k || 0)
		}
	}
...
(function (p) {
	...
	p.parser("parseVTT", function (d) {
		var b = {title: "", remote: "", data: []}, h = [], i = 0, g = 0, k, r;
		d = d.text.split(/(?:\r\n|\r|\n)/gm);
		g = d.length;
...
(function (p) {
	function e(b, h) {
		var i = b.substr(10).split(","), g;
		g = {start: l(i[h.start]), end: l(i[h.end])};
		if (g.start === -1 || g.end === -1)throw"Invalid time";
		var k = q.call(m, /\{(\\[\w]+\(?([\w\d]+,?)+\)?)+\}/gi, ""), r = k.replace, m;
		m = i.length;
		q = [];
	...
	p.parser("parseSSA", function (b) {
		var h = {title: "", remote: "", data: []}, i = [], g = 0, k;
		b = b.text.split(/(?:\r\n|\r|\n)/gm);
		for (k = b.length; g < k && b[g] !== "[Events]";)g++;
		var r = b[++g].substr(8).split(", "), m = {}, t, q;
		q = 0;
		for (t = r.length; q < t; q++)if (r[q] === "Start")m.start = q; else if (r[q] === "End")m.end = q; else if (r[q] === "Text")m.text = q;
...
(function (p) {
	...
	p.parser("parseSRT", function (d, b) {
		var h = {title: "", remote: "", data: []}, i = [], g = 0, k = 0, r, m, t, q, o;
		r = d.text.split(/(?:\r\n|\r|\n)/gm);
...
(function (p, e) {
	...
	e.player("youtube", {_canPlayType: l});
